...
|---|---|---|---|
| `parse_server_runtime`, `parse_client_runtime`, `DesktopMode` | `shared/src/cli.rs` | Resolve Clap flags, direct environment variables, and `_FILE` environment indirection before layering them over TOML configuration; client parsing expands all route sessions and selects automatic/required/headless desktop integration. | Binary entry points and containers |
...
| `ServerConfig`, `SniPassthroughRoute`, `ClientConfig`, `BackendRule` | `shared/src/config.rs` | Define concrete typed component configuration, exact direct TLS/SNI passthrough routes, Caddy-like HTTP proxy defaults, and tunnel routing data; backend addresses may use `PORT` or `:PORT` as loopback shorthand. | CLI, server, client |
