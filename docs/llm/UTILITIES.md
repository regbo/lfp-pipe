...
the ingress or client crates.

| Public utility | Location | Responsibility | Main consumers |
|---|---|---|---|
| `parse_ingress_runtime`, `parse_client_runtime`, `DesktopMode` | `shared/src/cli.rs` | Resolve Clap flags, direct environment variables, and `_FILE` environment indirection before layering them over TOML configuration; client parsing expands all route sessions and selects automatic/required/headless desktop integration. | Binary entry points and containers |
...
| `config_source::run_central` | `client/src/config_source.rs` | Fetch, authenticate, validate, and continuously synchronize centrally managed route documents with SSE plus polling, acknowledge the applied revision, and retain the last valid running configuration. | Headless and desktop tunnel clients |
...
| `IngressConfig`, `SniPassthroughRoute`, `ClientConfig`, `BackendRule` | `shared/src/config.rs` | Define concrete typed component configuration, exact direct TLS/SNI passthrough routes, Caddy-like HTTP proxy defaults, and tunnel routing data; backend addresses may use `PORT` or `:PORT` as loopback shorthand. | CLI, ingress, client |
| `ClientConfigDefaults`, `ClientRouteConfig`, `ClientPathRouteConfig`, `CentralClientBootstrap`, `load_client_configs` | `shared/src/config.rs` | Inherit shared settings into independently authenticated hostname sessions and path-specific backends, including fail-closed HTTP-only routes through `tcp_passthrough = false`, or select a minimal central-control-plane bootstrap while retaining the legacy format. | Multi-route tunnel client |
...
| `extract_http_host`, `looks_like_http_prefix` | `shared/src/http.rs` | Detect supported HTTP prefixes and extract a normalized Host header. | Ingress routing |
...
| `copy_bidirectional_with_mode` | `shared/src/io.rs` | Select splice or bounded buffered bidirectional TCP forwarding. | Ingress and client relays |
...
| `connect_nats` | `shared/src/nats.rs` | Establish a deterministic NATS control-plane connection. | Ingress and client startup |
...
| `PrefixEnvelope` | `shared/src/prefix.rs` | Encode and validate the versioned callback-to-ingress binding prefix. | Client callback and ingress |
| `ConnectionRequest`, `ConnectionClaim`, `ConnectionClaimAck` | `shared/src/protocol.rs` | Define JSON messages exchanged over NATS. | Ingress and client control plane |
...
| `extract_sni`, `validate_tls_record_header` | `shared/src/tls.rs` | Validate TLS handshake records and extract ClientHello SNI. | Ingress routing |
